...
if (any(snps.to.keep == "PASS")) {
    ...
    if (opt$interaction == "none") {
        for (p in geno.df$pos) {
            ...
            res_manova <- tryCatch( {summary(manova(Y ~ ., data.frame(cov.df, "GT" = snp)))}, 
                                         error = function(e){return(NA)} )
            if (is.null(mvfit)) {
                warning(sprintf("SNP %s skipped",  subset(geno.df, pos == p)$variant))
                next
            }
            out.df <- rbind(out.df, c(t(rec), mvfit$aov.tab[1, 4:6],res_manova$aov.tab[1, 4:6]))
        }
    } else {
        ...
        for (p in geno.df$pos) {
            ...
            mvfit <- tryCatch(manta(fm,  data = data.frame(cov.df, "GT" = snp), type = "II", transform = opt$transform, 
                              ...
                              error = function(e) NULL)
            res_manova[snp] <- tryCatch( {summary(manova(Y ~ ., data.frame(cov.df, "GT" = snp)))}, 
                                         error = function(e){return(NA)} )
            if (is.null(mvfit)) {
                warning(sprintf("SNP %s skipped",  subset(geno.df, pos == p)$variant))
                next
            }
            out.df <- rbind(out.df, c(t(rec), mvfit$aov.tab[1:3, 4:6],res_manova$aov.tab[1:3, 4:6]))
        }
    }
